...
Правила для всего текста в репозитории - контент постов, интерфейсные строки,
комментарии, документация:

- **Никаких длинных тире.** Ни `—`, ни `–`. Только обычный дефис `-`.
- **Никакой `ё`.** Пишем `е`: «еж», «еще», «свеклы».
- Проверка: `grep -rn '[\u2014\u2013\u0451\u0401]' src scripts main.config.ts README.md CLAUDE.md`
  должен молчать. Сам этот раздел из проверки исключен: он обязан называть
  запрещенные символы.
...
bun run verify      # все сразу (lint + check + check:seo)
```
