...
description: "388 gradients where the catalogue is a stylesheet and the copy-ready code is derived from it at build time. Architecture, search, Tailwind."
categories: ["astro", "tailwind", "design-systems"]
pubDate: 2026-08-21
...
Inside there are 388 gradients and one architectural decision that everything
else follows from.
...
<Figure
	src={home}
	alt="The color.xima.work home page: the headline, a button to 388 gradients and a strip of previews"
/>
...
<Figure
	src={cards}
	alt="The /cards page: search, tag chips with counts and a grid of cards with a copy button"
/>
...
The decision is arguable but honest: nobody is going to write close to four
hundred descriptions by hand, and deterministic generation at least keeps the text stable between builds
...
Because the catalogue is a stylesheet, the prune came down to deleting rules from
the CSS and entries from the index. Neither previews nor snippets had to be
rebuilt: they are derived from that same file anyway.
...
The honesty in the data comment is worth noting: Pantone picks colours for
physical inks and textiles, so every hex here is the widely published sRGB
approximation of the TCX chip rather than an exact conversion, and two sources
can disagree by a point or two.

Each chip also becomes a three-stop CSS gradient - a tint, the chip, then a
shade - so a year can be dropped straight into a layout.

<Figure
	src={pantone}
	alt="The Pantone section: the 2026 colour Cloud Dancer as a large swatch with a 27-year strip below"
/>
